...
app.controller("ctlEmployee", function($scope, $http, $route, $routeParams, $location, $window, $postMessage, $rootScope, $exceptionHandler, debug, AuthService){
	...
	$scope.getSelectedClientIndex = function() {
 		if ($scope.tcid.employee.maindata == undefined) { console.log("returning flase get selected client index");  return false; }
	...
	$scope.getSelectedCompanyIndex = function() {
		if ($scope.tcid.employee.maindata == undefined) { console.log("returning flase get selected company index"); return false; }
...
function ValidURL(str) {
 ...
 $scope.$broadcast('show-errors-check-validity');
		...
		console.log($scope);
		//api2fa here is hostname of the plugin
		var responsePromise = $http.post($scope.apiURL+'/api/v1/api_employees/save?u='+$scope.tcid.username+'&apikey='+$scope.tcid.api_key + '&companyid='+$scope.tcid.employee.maindata.companyid+'&locationid='+$scope.tcid.employee.maindata.locationid+'&api2fa='+window.location.hostname, $scope.tcid.employee.maindata, {});
		responsePromise.success(function(dataFromServer, status, headers, config) {
			...
			if (dataFromServer.SUCCESS) {
				...
				//TODO: Require SSL
				if($scope.args.callback_url !== false && ValidURL($scope.args.callback_url)){ //&& && protocol==https
					try{
		...
		responsePromise.error(function(data, status, headers, config) {
			$exceptionHandler({'data':data,'status':status,'headers':headers,'config':config});
			alert("Failed Submission. We have notified our administrators.");
		});
	}
	...
	setEmployee = function(user_provided_data, currentuser) {
		...
		console.log(currentuser.client.clientid);
		console.log(currentuser.client.name);
		emp.maindata.client.id=currentuser.client.clientid;//typeof user_provided_data.clientid!='undefined' ? user_provided_data.clientid : null;
		emp.maindata.client.name=typeof currentuser.client.name!='undefined' ?  currentuser.client.name : '';
		
		emp.maindata.company.id=typeof user_provided_data.companyid!='undefined' ? user_provided_data.companyid : currentuser.company.companyid;
        ...
        emp.maindata.location.id = typeof user_provided_data.locationid!='undefined' ? user_provided_data.locationid : currentuser.company.companyid;
        emp.maindata.location.name = '';
...
/*
        ...
        emp.maindata.client.name = src.maindata.client_name;
        emp.maindata.company.id = src.maindata.companyid;
        emp.maindata.company.name = typeof src.maindata.legal_name == 'undefined' ? '': src.maindata.legal_name[0];  //LocalStorage wont have this data so conditionally set them
        emp.maindata.location.id = src.maindata.locationid;
        emp.maindata.location.name = typeof src.maindata.location_name == 'undefined' ? '': src.maindata.location_name[0]; //LocalStorage wont have this data so conditionally set them

        emp.maindata.clientid = function() {
            return emp.maindata.client.id
        };
        emp.maindata.companyid = function() {
            return emp.maindata.company.id;
        };
        emp.maindata.locationid = emp.maindata.location.id;
...
/*
   defEmployee = function() {
        ...
        emp.maindata.stateid = null;
        emp.maindata.zip = '';
        emp.maindata.address = '';
        emp.maindata.address2 = '';
        //emp.maindata.countyid=null;
        emp.maindata.dob = null;

        emp.maindata.client = {};
        emp.maindata.company = {};
        emp.maindata.location = {};
        emp.maindata.client.id = 0;
        emp.maindata.client.name = '';
...
*/	
	...
	$scope.$on('$messageIncoming', function(event, args) {
        ...
        switch(args.plugin_type.toLowerCase()) {
			...
			//$scope.tcid=getCurrentUser(); //must go after uname and apikey
			
			$scope.tcid.employee=getCurrentUser();
			console.log("RUN CCL");
